...
func TestAuthorList(t *testing.T) {
	...
	mock := NewMockModeler(mctrl)
	gomock.InOrder(
		mock.EXPECT().ListAuthorsByIDs(nil, nil).Return(nil, errors.New("list error")),
		mock.EXPECT().Close(),
		mock.EXPECT().ListAuthorsByIDs(nil, nil).Return([]*model.Author{&model.Author{ID: 123, Name: "foo"}, &model.Author{ID: 456, Name: "bar"}}, nil),
		mock.EXPECT().Close(),
	)
	service := goa.New("my-inventory-test")
	logbuf := &strings.Builder{}
