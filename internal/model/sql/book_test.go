...
ON DUPLICATE KEY UPDATE update_ts = VALUES\(update_ts\)`
	...
	isbnqry := `SELECT book_id, isbn, name from books where isbn = \?`
	mock.
		ExpectQuery(isbnqry).
		WithArgs("foo").
		WillReturnError(errors.New("duplicate error"))
	mock.ExpectQuery(isbnqry).WithArgs("foo").WillReturnRows(sqlmock.NewRows([]string{"book_id", "isbn", "name"}).AddRow(1, "foo", "bar"))
...
func TestGetBookByID(t *testing.T) {
	...
	qry := `SELECT book_id, isbn, name from books where id = \?`
	mock.
		ExpectQuery(qry).
		WithArgs(123).
	...
	mock.
		ExpectQuery(qry).
		WithArgs(123).
		WillReturnRows(sqlmock.NewRows([]string{"book_id", "isbn", "name"}))
	mock.
		ExpectQuery(qry).
		WithArgs(123).
		WillReturnRows(sqlmock.NewRows([]string{"book_id", "isbn", "name"}).AddRow("foo", "bar", "baz"))
	mock.
		ExpectQuery(qry).
		WithArgs(123).
		WillReturnRows(sqlmock.NewRows([]string{"book_id", "isbn", "name"}).AddRow(1, "foo", "bar").RowError(0, errors.New("scan error")))
	mock.
		ExpectQuery(qry).
		WithArgs(123).
		WillReturnRows(sqlmock.NewRows([]string{"book_id", "isbn", "name"}).AddRow(1, "foo", "bar"))
	...
	tests := []struct {
		...
		{"scan conversion error", nil, errors.New(`sql: Scan error on column index 0, name "book_id": converting driver.Value type string ("foo") to a int64: invalid syntax`)},
...
func TestGetBookByISBN(t *testing.T) {
	...
	qry := `SELECT book_id, isbn, name from books where isbn = \?`
	mock.
		ExpectQuery(qry).
		WithArgs("foo").
		WillReturnError(errors.New("query error"))
...
func TestGetBookByName(t *testing.T) {
	...
	qry := `SELECT book_id, isbn, name from books where name = \?`
	mock.
		ExpectQuery(qry).
		WithArgs("foo").
		WillReturnError(errors.New("query error"))
...
func TestGetBookList(t *testing.T) {
	...
	qry := `SELECT book_id, isbn, name FROM books`
	mock.
		ExpectQuery(qry).
		WillReturnError(errors.New("query error"))
	mock.
...
func TestUpdateBook(t *testing.T) {
	...
	qry := `UPDATE books set name = \?, update_ts = NOW\(\) where book_id = \?`
...
func TestDeleteBook(t *testing.T) {
	...
	qry := `DELETE FROM books where book_id = \?`
