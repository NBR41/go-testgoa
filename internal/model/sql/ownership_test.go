...
func TestGetOwnershipList(t *testing.T) {
	...
	qry := `SELECT b.book_id, b.isbn, b.name FROM ownerships u JOIN books b USING\(book_id\) where user_id = \?`
	mock.ExpectQuery(qry).WithArgs(123).WillReturnError(errors.New("query error"))
	mock.ExpectQuery(qry).WithArgs(123).WillReturnRows(sqlmock.NewRows([]string{"book_id", "isbn", "name"}))
	mock.ExpectQuery(qry).WithArgs(123).WillReturnRows(sqlmock.NewRows([]string{"book_id", "isbn", "name"}).AddRow("foo", "bar", "baz"))
	mock.ExpectQuery(qry).WithArgs(123).WillReturnRows(sqlmock.NewRows([]string{"book_id", "isbn", "name"}).AddRow(456, "foo", "bar").RowError(0, errors.New("scan error")))
	mock.ExpectQuery(qry).WithArgs(123).WillReturnRows(sqlmock.NewRows([]string{"book_id", "isbn", "name"}).AddRow(456, "foo", "bar"))
...
func TestUpdateOwnership(t *testing.T) {
	...
	qry := `UPDATE ownerships set update_ts = NOW\(\) where user_id = \? and book_id = \?`
	mock.ExpectExec(qry).WithArgs(123, 456).WillReturnError(errors.New("query error"))
...
func TestDeleteOwnership(t *testing.T) {
	...
	qry := `DELETE FROM ownerships where user_id = \? and book_id = \?`
	mock.ExpectExec(qry).WithArgs(123, 456).WillReturnError(errors.New("query error"))
	mock.ExpectExec(qry).WithArgs(123, 456).WillReturnResult(sqlmock.NewErrorResult(errors.New("result error")))
	mock.ExpectExec(qry).WithArgs(123, 456).WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectExec(qry).WithArgs(123, 456).WillReturnResult(sqlmock.NewResult(0, 1))
