...
func TestListPrintsByIDs(t *testing.T) {
	var collectionID, editorID, seriesID int = 1, 2, 3
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("an error '%s' was not expected when opening a stub database connection", err)
	}
	qry := `SELECT DISTINCT print.id, print.name FROM print WHERE 1`
	...
	qry = escapeQuery(`SELECT DISTINCT print.id, print.name FROM print JOIN edition ON (edition.print_id = print.id) JOIN collection ON (edition.collection_id = collection.id) JOIN book ON (edition.book_id = book.id) WHERE 1 AND edition.collection_id = ? AND collection.editor_id = ? AND book.series_id = ?`)
	mock.ExpectQuery(qry).WithArgs(collectionID, editorID, seriesID).WillReturnRows(sqlmock.NewRows([]string{"id", "name"}).AddRow(1, "foo"))
	...
	tests := []struct {
		...
		{"valid", []*int{nil, nil, nil}, []model.Print{model.Print{ID: 1, Name: "foo"}}, nil},
		{"valid with filter", []*int{&collectionID, &editorID, &seriesID}, []model.Print{model.Print{ID: 1, Name: "foo"}}, nil},
	}

	for i := range tests {
		v, err := m.ListPrintsByIDs(tests[i].params[0], tests[i].params[1], tests[i].params[2])
		if err != nil {
			if tests[i].err == nil {
				t.Errorf("unexpected error for [%s], [%v]", tests[i].desc, err)
				continue
